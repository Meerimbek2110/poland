...
import { LiaShoppingBasketSolid } from "react-icons/lia";
import { Link } from "react-router-dom";
import { CartContext } from "../../contex/CartContex"; // ✅ контекст корзины
...
const dishesData = [
  { id: 1, name: "Котлета с картофелем по-деревенски.", weight: "350 г", desc: "По семейному рецепту: нежнейшее рубленое мясо с экологически чистым ферме...", price: 430, img: gorbluda1 },
  { id: 2, name: "Оджаури из телятины", weight: "430 г", desc: "Невероятно ароматная лепёшка с сыром сулугуни внутри и снаружи", price: 490, img: gorbluda2 },
  { id: 3, name: "Долма", weight: "350 г", desc: "Невероятно ароматная начинка из рубленого мяса и свежей зелени, завернутая...", price: 410, img: gorbluda3 },
  { id: 4, name: "Чихохбили", weight: "350 г", desc: "Невероятно ароматная начинка из рубленого мяса и свежей зелени, завернутая...", price: 490, img: gorbluda4 },
  { id: 5, name: "Лобио по-имеретински", weight: "430 г", desc: "Невероятно ароматная лепёшка с сыром сулугуни внутри и снаружи", price: 490, img: gorbluda5 },
  { id: 6, name: "Стейк из лосося с овощами", weight: "430 г", desc: "Невероятно ароматная лепёшка с сыром сулугуни внутри и снаружи", price: 1200, img: gorbluda6 },
];

function Gorbluda() {
  const [sortOrder, setSortOrder] = useState("asc");
  const { addToCart } = useContext(CartContext); // ✅ корзина
      ...
      <div className="gorbluda-grid">
        {sortedDishes.map((dish) => (
          <div key={dish.id} className="gorbluda-card">
            ...
            <div className="gorbluda-footer">
              <span className="gorbluda-price">{dish.price} ₽</span>
              <button className="gorbluda-btn" onClick={() => addToCart({ ...dish, quantity: 1 })}>
                <LiaShoppingBasketSolid className="basket-icon" />
              </button>
            </div>
          </div>
