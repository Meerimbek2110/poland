...
import { LiaShoppingBasketSolid } from "react-icons/lia";
import { Link } from "react-router-dom";
import { CartContext } from "../../contex/CartContex"; // подключаем корзину
...
const buterbrodyData = [
  { id: 1, name: "Клаб-сэндвич", weight: "320 г", desc: "Тостовый хлеб, курица, бекон, помидор, салат, майонез", price: 350, img: buterbrod1 },
  { id: 2, name: "Сэндвич с ростбифом", weight: "280 г", desc: "Ржаной хлеб, нежный ростбиф, руккола, соус, огурец", price: 410, img: buterbrod2 },
  { id: 3, name: "Бутерброд с тунцом", weight: "250 г", desc: "Цельнозерновой хлеб, тунец, яйцо, майонез, свежий салат", price: 290, img: buterbrod3 },
  { id: 4, name: "Бутерброд с лососем и авокадо", weight: "220 г", desc: "Хлеб, слабосоленый лосось, авокадо, творожный сыр, кунжут", price: 520, img: buterbrod4 },
  { id: 5, name: "Вегетарианский сэндвич", weight: "270 г", desc: "Безглютеновый хлеб, хумус, гриль-овощи, сыр, зелень", price: 340, img: buterbrod5 },
  { id: 6, name: "Сэндвич с индейкой", weight: "300 г", desc: "Чиабатта, отварная индейка, сыр, помидор, соус песто", price: 380, img: buterbrod6 },
];

function Buter() {
  const [sortOrder, setSortOrder] = useState("asc");
  const { addToCart } = useContext(CartContext); // функция добавления в корзину

  const sortedButerbrody = [...buterbrodyData].sort((a, b) => sortOrder === "asc" ? a.price - b.price : b.price - a.price);
        ...
        {sortedButerbrody.map((item) => (
          <div key={item.id} className="gorbluda-card">
            ...
            <div className="gorbluda-footer">
              ...
              <button className="gorbluda-btn" onClick={() => addToCart({ ...item, quantity: 1 })}>
                <LiaShoppingBasketSolid className="basket-icon" />
              </button>
            </div>
          </div>
